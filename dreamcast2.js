...
(function($) {
  ...
  var Game = function(sel, options, callback) {
    ...
    this.playSound = function(sound) {
      var func = this.audioEnabled
        ? this["_" + this.soundMode + "PlaySound"]
        : this._noopPlaySound;
  ...
  Game.prototype.popScene = function() {
    ...
    if (previousScene) {
      if (previousScene.layer) {
        previousScene.layer.hasAttribute("display") &&
  ...
  Game.prototype._html5PreloadSounds = function(sounds) {
    $.each(sounds, function(name, file) {
      ...
      $("body").append(
        '<audio src="' +
          file +
          '.wav" autobuffer="autobuffer" preload="auto" id="' +
          id +
          '" />'
      );
    });
  ...
  Scene.prototype.addActor = function(actor, layer) {
    ...
    actor.scene = this;
    actor.layer =
      layer && this.layers[layer] ? this.layers[layer] : this.game.svg.root();
  ...
  var Sprite = function(options) {
    ...
    this.animateTime = 0;
    this.mode =
      svgweb.config && svgweb.config.use == "flash" ? "embed" : "clip";
  };
  ...
  Sprite.prototype.update = function(elapsed) {
    if (this.enabled) {
      if (!this.element && this.scene) {
        ...
        if (this.mode == "clip") {
          this.element = game.svg.image(
            this.layer,
            0,
            0,
            this.frameSize.width * this.frameCount,
            this.frameSize.height,
            this.image,
            {
              "clip-path":
                "url(#" +
                game.getMask(this.frameSize.width, this.frameSize.height) +
                ")"
            }
          );
          ...
          this.imageElement = game.svg.image(
            this.embeddedSvg,
            0,
            0,
            this.frameSize.width * this.frameCount,
  ...
  Sprite.prototype.updatePosition = function() {
    if (this.moveTime > this.moveDuration) {
      ...
      var newX = this.easing(
        this.moveTime / this.moveDuration,
        this.moveTime,
        this.moveStartPos.x,
        this.moveDestPos.x - this.moveStartPos.x,
        this.moveDuration
      );
      var newY = this.easing(
        this.moveTime / this.moveDuration,
        this.moveTime,
        this.moveStartPos.y,
        this.moveDestPos.y - this.moveStartPos.y,
        this.moveDuration
      );
